...
# Minimum R-multiples required for each TP level (risk = entry - stop_loss)
# TP1 must give at least 1:1, TP2 at least 2:1, TP3 at least 3:1
MIN_R_MULTIPLES = [1.0, 2.0, 3.0]

# Maximum R-multiples allowed (levels too far = impractical targets)
# PDH at +15R is unrealistic for intraday - cap at 4R/6R/8R
MAX_R_MULTIPLES = [4.0, 6.0, 8.0]

# Maximum percent distance from entry (absolute cap regardless of R)
# Prevents crazy targets on tight-stop setups
MAX_PCT_FROM_ENTRY = [0.35, 0.60, 0.90]  # 35%, 60%, 90%

# Minimum risk per share (prevents division issues with tight stops)
...
class TakeProfitPlanner:
    ...
    def find_swing_points(candles: List[Dict], lookback: int = 3) -> Tuple[Optional[float], Optional[float], Optional[float]]:
        """
        Find ABC swing points for Fibonacci extension calculation.
        A = swing low, B = swing high after A, C = pullback low after B
        """
    ...
    def detect_confluence(levels: List[Tuple[float, str]], tolerance: float = CONFLUENCE_TOLERANCE) -> List[Tuple[float, str, bool]]:
        """
        Detect confluence zones where multiple levels align within tolerance.
        Returns list of (price, source, is_confluence)
        """
        if not levels:
            return []
    ...
    def calculate_tp_levels(
        ...
        """
        Calculate TP1, TP2, TP3 and Stop Loss using technical levels with R:R guardrails.
        
        Priority (highest to lowest) - levels with LOWER priority number are chosen first:
        1. Previous Day High (PDH) - priority 1
        2. Pre-market High (PMH) - priority 2
        3. Opening Range High - priority 3
        4. Intraday Session High - priority 4
        5. Fibonacci Extensions (1.618, 2.0, 2.618) - priority 5
        6. Psychological Levels - priority 6
        
        R:R Guardrails:
        - TP1: Min 1R, Max 4R, Max 35% from entry
        - TP2: Min 2R, Max 6R, Max 60% from entry
        - TP3: Min 3R, Max 8R, Max 90% from entry
        - Levels that fail these checks are skipped
        
        Returns: (tp1, tp2, tp3, stop_loss)
        """
        PRIORITY_MAP = {
            "PDH": 1,
            "PMH": 2,
        ...
        # STEP 4: Helper to check if level is valid for a given TP slot
        def is_valid_for_slot(level_price: float, slot_index: int) -> bool:
            """Check if level meets R:R requirements for TP1/TP2/TP3 (slot 0/1/2)"""
        ...
        # STEP 6: If no technical levels at all, use pure fallback
        if not all_levels:
        ...
        # STEP 8: Sort by priority, then select TP levels with R:R validation
    ...
    def calculate_fib_retracements(cls, swing_low: float, swing_high: float) -> List[Tuple[float, str]]:
        """
        Calculate Fibonacci retracement levels for entry.
        Retracements are calculated from swing high back toward swing low.
        """
    ...
    def get_psychological_support_levels(cls, current_price: float, min_price: float = 0.10, max_levels: int = 5) -> List[Tuple[float, str]]:
        """Generate psychological price levels BELOW current price (support)."""
        levels = []
    ...
    def calculate_entry_levels(
        ...
        """
        Calculate the best entry point using confluence of support levels.
        
        Support levels considered (for BUY entries, looking for pullback zones):
        - PDL (Previous Day Low) - strong support
        - PML (Pre-Market Low) - intraday support
        - VWAP - dynamic support
        - Fib retracements (38.2%, 50%, 61.8%) - pullback zones
        - EMA 8/21 - moving average support
        - Psychological levels - round numbers below price
        
        Returns: (best_entry_price, confluence_sources, all_support_levels)
        """
        SUPPORT_PRIORITY = {
            "PDL": 1,
            "PML": 2,
        ...
        # If no levels found, return current price with no confluence
        if not all_levels:
            return (current_price, "Market", [])
        
        # Detect confluence and boost priority
        ...
        while i < len(sorted_by_price):
            ...
            while j < len(sorted_by_price):
                next_price, next_source, next_priority = sorted_by_price[j]
                # 1% tolerance for confluence detection
        ...
        # Sort by effective priority (lower = better), then by distance to current price (closer = better)
        sorted_levels = sorted(confluent_levels, key=lambda x: (x[3], current_price - x[0]))
        
        # Return the best entry (highest priority with confluence)
        best = sorted_levels[0]
        best_price = round(best[0], 4)
        best_sources = best[1]
        
        # Format all levels for display
...
class TradeSignalManager:
    ...
    async def update_price(self, symbol: str, current_price: float, timestamp: Optional[datetime] = None) -> List[ClosedTrade]:
        """
        Keep this function for tick-level updates (single price).
        For bar-level updates where TP/SL may occur intrabar, use update_price_bar() for better detection.
        """
        now = timestamp or datetime.now(timezone.utc)
    ...
    async def update_price_bar(self, symbol: str, open_p: float, high: float, low: float, close_p: float, timestamp: Optional[datetime] = None) -> List[ClosedTrade]:
        """
        Use this when you receive bar updates (e.g., 1m bars). This will attempt to detect intrabar
        TP/SL hits using the bar high/low. NOTE: Without tick data we cannot know the order within the bar.
        Heuristic used:
          - If only TP or SL is inside the bar, close accordingly.
          - If both TP and SL fall within the same bar:
              * If close >= TP => assume TP happened later in bar, close at TP.
              * Elif close <= SL => assume SL happened later, close at SL.
              * Else use bar direction: if close > open (up bar) assume TP first, else SL first.
          This is a best-effort heuristic — tick-level data is needed for perfect ordering.
        """
        now = timestamp or datetime.now(timezone.utc)
        closed: List[ClosedTrade] = []
        ...
        async with self._lock:
            ...
            for pid in ids:
                ...
                if tp_hit and not sl_hit:
                    ...
                    # both within same bar: best-effort resolution
                    ...
                    # neither TP nor SL intrabar; check trailing or timeout using close price
                    # update trailing with close
